...
Errors in a filter fall into five categories. Each has a distinct API surface.

**1. Callout init failure.** `handle.HttpCallout(...)` returns a non-success
`HttpCalloutInitResult` synchronously before the callout is sent. Common causes:
cluster name not found in CDS, missing required headers. The callback is never
called. Check the result and call `SendLocalResponse` immediately.

```go
init, _ := handle.HttpCallout("auth", headers, nil, 500, callbackReceiver)
if init != shared.HttpCalloutInitSuccess {
    handle.SendLocalResponse(503, nil, errBody, "callout_init_fail")
    return shared.HeadersStatusStop
}
return shared.HeadersStatusStop // callout in flight: Envoy pauses the request
```

**2. Callout network failure.** The callout was sent but the upstream reset the
connection or exceeded the response buffer limit. `OnHttpCalloutDone` fires with
`result == HttpCalloutReset` or `HttpCalloutExceedResponseBufferLimit`. Send 502.

**3. Callout upstream error.** The callout completed but the upstream returned a
non-2xx status. Read `:status` from the callout response headers and decide:
forward the status, remap it (5xx -> 403 to hide internals), or pass through.

**4. Envoy local reply (OnLocalReply).** Envoy itself sends a local reply when
an upstream timeout fires, a circuit breaker opens, a rate limiter trips, or a
buffer overflows. The filter's `OnLocalReply` method receives the response code
and a `details` string identifying the reason. You can read and modify response
headers here (e.g. add `x-error-details`). Return `LocalReplyStatusContinue`.
This is not called for replies generated by the filter's own `SendLocalResponse`.

**5. Response flags (OnStreamComplete).** `GetAttributeString(AttributeIDResponseFlags)`
returns the final Envoy response flags string after the request resolves.
Common flags: `UF` (upstream connection failure), `UC` (upstream connection
termination), `UT` (upstream timeout), `DC` (downstream disconnect). Read these
in `OnStreamComplete` to emit per-flag counters and correlate with the request path.

See [examples/error-handling](examples/error-handling/) for a runnable filter
covering all five paths with unit tests and an envoy.yaml.
